...
In order to build and develop on Kahunabooking foundations you need the following:
...
Kahunabooking uses [PostgreSQL](https://www.postgresql.org) to store data, so you will need the database running to use the application. By default, Kahunabooking uses the `kahunabooking` database using the `postgres` user (empty password), connecting to a server running on `localhost:5432`. This can be customised in the `application.conf` file.
