...
```
java -jar backend/target/scala-2.12/kahunabooking.jar
```
...
Use [Helm](https://helm.sh/) to easily deploy Kahunabooking into [Kubernetes](https://kubernetes.io/) cluster.
...
### Fetch and Customize Kahunabooking chart

```
helm fetch softwaremill/kahunabooking --untar
```

### Install Kahunabooking chart

```
helm install --generate-name kahunabooking
```

Please see [Kahunabooking Helm Chart documentation](https://github.com/softwaremill/kahunabooking/blob/master/helm/kahunabooking/README.md) for more information, including configuration options.


## Heroku

Kahunabooking-based applications can be easily deployed to [Heroku](https://www.heroku.com).
...
Kahunabooking already includes the Postgres driver and properly recognizes the `DATABASE_URL` environment variable that is set by Postgres Heroku.

Now you can deploy your app. Kahunabooking includes an sbt task which will build the fat-jar and upload it:
